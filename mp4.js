...
function clearMoov(moov, tracks = null) {
...
async function convertToFragmented(videoUrl, videoEl) {
    ...
    let read = async () => {
        ...
        if (foundMoof) {
            ...
            let mdatEnd = 0;
            let minDuration = 5 * readers[trackId - 1].timeScale;
            while (builder.duration() < minDuration) {
                let sample = readers[trackId - 1].readSampleInfo();
...
async function fragmentedMp4Test(videoUrl, videoEl) {
    let mimeCodec = 'video/mp4; codecs="avc1.4d4020, mp4a.40.2"'; // TODO
    if (!MediaSource.isTypeSupported(mimeCodec)) {
        throw 'Unsupported MIME type or codec: ' + mimeCodec;
    }
    ...
    let reader = (await fetch(videoUrl)).body.getReader();
    let read = async () => {
...
window.addEventListener('DOMContentLoaded', async (ev) => {
    ...
    videoUrl = 'so37187156.mp4';
    convertToFragmented(videoUrl, videoEl);
    // fragmentedMp4Test(videoUrl, videoEl);
}, { once: true });
